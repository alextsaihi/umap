...
export default function Page() {
  ...
  const [selectedBuffers, setSelectedBuffers] = useState<Set<string>>(
    new Set()
  );
  const [selectedIncubationTime, setSelectedIncubationTime] = useState<
    Set<number>
  >(new Set());
    ...
    const fetchData = async () => {
      ...
      try {
        const plotResponse = await fetch(
          `http://127.0.0.1:8000/api/umapplotpoint/`
        );
        if (!plotResponse.ok) {
        ...
        plotData.forEach((point: UmapPlotPoint) => {
          ...
          if (point.sample.dataset && point.sample.dataset.id) {
            datasetSet.add(point.sample.dataset.id);
          }
        });
        // default select all
        ...
        const sortedIncubationTimes = Array.from(incubationTimeSet).sort(
          (a, b) => a - b
        );
        setSelectedIncubationTime(new Set<number>(sortedIncubationTimes));

        const signalResponse = await fetch(
          `http://127.0.0.1:8000/api/samplesignal/`
        );
  ...
  const filteredPlotData = plotData.filter((point) => {
    ...
    const datasetIncluded =
      selectedDatasets.size === 0 || selectedDatasets.has(sample.dataset.id);
    const donorIncluded =
      selectedDonors.size === 0 || selectedDonors.has(sample.metadata.donor);
    const bufferIncluded =
      selectedBuffers.size === 0 || selectedBuffers.has(sample.metadata.buffer);
    const incubationIncluded =
      selectedIncubationTime.size === 0 ||
      selectedIncubationTime.has(sample.metadata["incubation time (hr)"]);
    const targetIncluded =
      selectedTarget === null ||
      sampleSignals.some(
        (signal) =>
          signal.sample.id === sample.id && signal.target.id === selectedTarget
      );
  ...
  const colors = filteredPlotData.map((point) => {
    const signal = sampleSignals.find(
      (signal) =>
        signal.sample.id === point.sample.id &&
        signal.target.id === selectedTarget
    );
  ...
  const datasetOptions = Array.from(
    new Set(plotData.map((point) => point.sample.dataset.id))
  ).sort();
  const donorOptions = Array.from(
    new Set(plotData.map((point) => point.sample.metadata.donor))
  ).sort();
  const bufferOptions = Array.from(
    new Set(plotData.map((point) => point.sample.metadata.buffer))
  );
  const incubationTimeOptions = Array.from(
    new Set(
      plotData.map((point) => point.sample.metadata["incubation time (hr)"])
    )
  ).sort((a, b) => a - b);
      ...
      <div className="flex flex-wrap mb-4 gap-4 justify-center">
          ...
          {incubationTimeOptions.map((time) => (
            <Label key={time} className="block">
              <Checkbox
                ...
                onChange={(e) => {
                  const newSelectedIncubationTime = new Set(
                    selectedIncubationTime
                  );
                  if (e.target.checked) {
        ...
        <div className="flex flex-col">
          ...
          <Select
            ...
            onChange={(e) =>
              setSelectedTarget(e.target.value ? Number(e.target.value) : null)
            }
          >
            {targets.map((target) => (
